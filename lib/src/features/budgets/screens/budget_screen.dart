...
class _BudgetScreenState extends ConsumerState<BudgetScreen> {
  ...
  void _addOrEdit(BudgetModel? budget) async {
    final success = await showDialog<bool>(
      context: context,
      builder: (_) => AddBudgetDialog(budgetToEdit: budget),
    );
    if (success == true) {
      refreshBudgets(ref);
    }
  ...
  Future<void> _delete(BudgetModel b) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        title: Text('delete_budget_confirm'.tr() == 'delete_budget_confirm' ? 'Xác nhận xóa' : 'delete_budget_confirm'.tr()),
        content: Text('delete_budget_body'.tr() == 'delete_budget_body' ? 'Bạn chắc chắn muốn xóa hạn mức này?' : 'delete_budget_body'.tr()),
        actions: [
  ...
  double _getSpentAmount(int categoryId, List entries) {
    return (entries as List).where((e) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text('budget_management'.tr()),
      ),
      body: Column(
        children: [
          _buildMonthPicker(),
            ...
            child: budgetsAsync.when(
              data: (list) {
                final currentBudgets = list.where((b) {
                   return b.startDate.year == _selectedDate.year && b.startDate.month == _selectedDate.month;
                }).toList();
                ...
                return entriesAsync.when(
                  data: (entries) => ListView.builder(
                    ...
                    itemBuilder: (ctx, idx) {
                      ...
                      final cat = cats.where((c) => c.id == b.categoryId).firstOrNull;
              ...
              loading: () => const Center(child: CircularProgressIndicator()),
              error: (e, _) => Center(child: Text('Lỗi tải ngân sách: $e')),
            ),
          ),
        ],
      ),
      ...
      child: Row(
        ...
        children: [
          ...
          TextButton(
            ...
            child: Text(
              DateFormat('MMMM yyyy', context.locale.toString()).format(_selectedDate),
     ...
     return Center(
       child: Column(
         ...
         children: [
           ...
           Text(
             'no_budget_set'.tr() == 'no_budget_set' ? 'Bạn chưa đặt ngân sách cho tháng này' : 'no_budget_set'.tr(),
             style: const TextStyle(fontSize: 16, color: Colors.grey),
           ),
           const SizedBox(height: 16),
           ElevatedButton(onPressed: () => _addOrEdit(null), child: Text('set_budget_now'.tr() == 'set_budget_now' ? 'Thiết lập ngay' : 'set_budget_now'.tr()))
         ],
       ),
     );
  }
  ...
  Widget _buildBudgetCard(BudgetModel b, double spent, String catName) {
    ...
    final color = isOver ? Colors.red : (percent > 0.8 ? Colors.orange : Colors.green);
          ...
          children: [
            Row(
              ...
              children: [
                ...
                Row(
                  children: [
                    IconButton(icon: const Icon(Icons.edit_outlined, size: 20), onPressed: () => _addOrEdit(b)),
                    IconButton(icon: const Icon(Icons.delete_outline, size: 20, color: Colors.red), onPressed: () => _delete(b)),
                  ],
                ),
              ],
            ),
            ...
            if (isOver)
              Padding(
                ...
                child: Text(
                  '⚠️ Vượt ngân sách ${NumberFormat("#,###").format(spent - b.amount)} đ',
